...
def analyze_chunk_types():
    ...
    for point in all_points:
        if hasattr(point, 'payload') and point.payload:
            ...
            is_relation = ('entity_name' in point.payload and 
    ...
    for chunk_type, count in sorted(metadata_vs_impl.items(), key=lambda x: x[1], reverse=True):
        if chunk_type != 'unknown':
            percentage = (count / total_non_relation * 100) if total_non_relation > 0 else 0
    ...
    for entity_type in major_types:
        if entity_type in entity_chunk_analysis:
            ...
            if total_chunks > 0:
                ...
                print(f'  Metadata:         {metadata_count:>6,} ({metadata_count/total_chunks*100:>5.1f}%)')
                print(f'  Implementation:   {impl_count:>6,} ({impl_count/total_chunks*100:>5.1f}%)')
                if impl_count > 0:
    ...
    for chunk_type, sizes in content_size_analysis.items():
        if sizes and chunk_type != 'unknown':
            ...
            max_size = max(sizes)
            min_size = min(sizes)
            print(f'{chunk_type:<20}: Avg {avg_size:>6.0f}, Max {max_size:>7,}, Min {min_size:>4}')
    ...
    print(f'Metadata Storage Efficiency:  {metadata_total:,} metadata chunks for {impl_total:,} implementations')
...
def analyze_file_level_bloat():
    ...
    for ext in sorted(ext_entity_counts.keys()):
        ...
        print(f'{ext:<8}: {total_entities:>5,} entities in {file_count:>3} files (avg {avg_per_file:>5.1f}/file)')
    ...
    if md_files:
        ...
        print(f'Top 5 MD files by entities:')
...
def analyze_content_quality():
    ...
    generic_name_patterns = [
        'test_', 'temp_', 'tmp_', 'debug_', 'sample_', 'example_',
        'function_', 'method_', 'class_', 'var_', 'item_', 'data_',
        'new_', 'old_', 'backup_', 'copy_', 'draft_'
    ]
    ...
    for point in all_points:
        if hasattr(point, 'payload') and point.payload:
            ...
            # Skip relations
            is_relation = ('relation_target' in point.payload and 
                          'relation_type' in point.payload)
            if is_relation:
                continue
    ...
    duplicate_names = [(name, count) for name, count in entity_names.items() if count > 1]
    ...
    if duplicate_names:
        print(f'Top 10 duplicated names:')
    ...
    if generic_patterns:
        ...
        for pattern, count in sorted(generic_patterns.items(), key=lambda x: x[1], reverse=True):
    ...
    if content_lengths:
        ...
        very_short = len([l for l in content_lengths if l < 20])
        short_content = len([l for l in content_lengths if 20 <= l < 50])
        medium_content = len([l for l in content_lengths if 50 <= l < 200])
        long_content = len([l for l in content_lengths if l >= 200])
        ...
        print(f'  Very short (<20 chars):  {very_short:>5,} ({very_short/len(content_lengths)*100:>5.1f}%)')
        print(f'  Short (20-49 chars):     {short_content:>5,} ({short_content/len(content_lengths)*100:>5.1f}%)')
        print(f'  Medium (50-199 chars):   {medium_content:>5,} ({medium_content/len(content_lengths)*100:>5.1f}%)')
        print(f'  Long (200+ chars):       {long_content:>5,} ({long_content/len(content_lengths)*100:>5.1f}%)')
    ...
    potential_duplicates = [(sig, count) for sig, count in duplicate_content.items() if count > 1]
    potential_duplicates.sort(key=lambda x: x[1], reverse=True)
    
    if potential_duplicates:
        print(f'Potentially duplicated content signatures: {len(potential_duplicates):,}')
    ...
    if len(duplicate_names) > 100:
        ...
        quality_issues.append(f'MEDIUM: {len(duplicate_names):,} duplicate entity names')
    
    if generic_patterns:
        total_generic = sum(generic_patterns.values())
        if total_generic > 500:
            quality_issues.append(f'HIGH: {total_generic:,} entities with generic names')
        elif total_generic > 200:
            quality_issues.append(f'MEDIUM: {total_generic:,} entities with generic names')
    
    if content_lengths:
        very_short_pct = len([l for l in content_lengths if l < 20]) / len(content_lengths) * 100
        if very_short_pct > 20:
            quality_issues.append(f'HIGH: {very_short_pct:.1f}% entities have very short content')
        elif very_short_pct > 10:
            quality_issues.append(f'MEDIUM: {very_short_pct:.1f}% entities have very short content')
...
def analyze_cross_category_bloat():
    ...
    for point in all_points:
        if hasattr(point, 'payload') and point.payload:
            # Check if relation
            is_relation = ('relation_target' in point.payload and 
                          'relation_type' in point.payload and
            ...
            if is_relation:
                ...
                relation_type = point.payload.get('relation_type', '')
                ...
                elif entity_type in ['function', 'class', 'variable', 'import', 'interface']:
                    doc_vs_code_entities['code'] += 1
                elif entity_type in ['implementation_pattern', 'debugging_pattern', 'performance_pattern', 
                                   'configuration_pattern', 'architecture_pattern', 'integration_pattern',
                                   'knowledge_insight', 'active_issue', 'ideas', 'automation_pattern']:
    ...
    for category, count in sorted(doc_vs_code_entities.items(), key=lambda x: x[1], reverse=True):
    ...
    overall_relation_ratio = total_relations / total_entities if total_entities > 0 else 0
    print(f'Total Relations:         {total_relations:,}')
    print(f'Total Entities:          {total_entities:,}')
    print(f'Relations per Entity:    {overall_relation_ratio:.2f}')
    
    # Analyze which entities have the most relations
    high_relation_entities = [(entity, count) for entity, count in relation_sources.items() if count >= 5]
    ...
    # Focus on major entity types
    major_types = ['documentation', 'function', 'class', 'variable', 'implementation_pattern', 'debugging_pattern']
    ...
    for entity_type in major_types:
        if entity_type in manual_vs_auto_by_type:
            ...
            print(f'{entity_type:<20}: {total_type:>5,} total | {manual_count:>4,} manual ({manual_pct:>4.1f}%) | {auto_count:>4,} auto ({auto_pct:>4.1f}%)')
    ...
    if code_count > 0:
        ...
        if doc_to_code_ratio > 2.0:
            bloat_indicators.append(f'HIGH: Documentation entities {doc_to_code_ratio:.1f}x more than code')
        elif doc_to_code_ratio > 1.5:
            bloat_indicators.append(f'MEDIUM: Documentation dominance ({doc_to_code_ratio:.1f}:1 ratio)')
    
    # Relation bloat check
    if overall_relation_ratio > 1.0:
        print(f'Relation Density:         {overall_relation_ratio:.2f} relations/entity')
        if overall_relation_ratio > 1.5:
            bloat_indicators.append(f'HIGH: Excessive relations ({overall_relation_ratio:.1f} per entity)')
        elif overall_relation_ratio > 1.2:
            bloat_indicators.append(f'MEDIUM: High relation density ({overall_relation_ratio:.1f} per entity)')
    
    # Manual pattern bloat
    manual_patterns = doc_vs_code_entities.get('manual_patterns', 0)
    manual_pattern_pct = (manual_patterns / total_categorized * 100) if total_categorized > 0 else 0
    print(f'Manual Pattern Content:   {manual_pattern_pct:.1f}% of all entities')
    
    if manual_pattern_pct > 5.0:
        bloat_indicators.append(f'MEDIUM: High manual pattern content ({manual_pattern_pct:.1f}%)')
