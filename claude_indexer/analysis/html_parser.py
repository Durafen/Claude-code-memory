...
class HTMLParser(TreeSitterParser):
    ...
    def parse(self, file_path: Path) -> ParserResult:
        ...
        try:
            ...
            id_entities = self._extract_elements_with_ids(
            ...
            component_entities = self._extract_components(
                tree.root_node, content, file_path
            )
            entities.extend(component_entities)

            # Extract inline CSS entities from <style> tags
            inline_css_entities = self._extract_inline_css_entities(
                tree.root_node, content, file_path
            )
    ...
    def _extract_elements_with_ids(
        ...
        for element in self._find_nodes_by_type(root, ["element"]):
            ...
            if id_value:
                ...
                entity = Entity(
                    ...
                    metadata={"type": "html_element", "tag": tag_name, "id": id_value},
                )
        ...
        for element in self._find_nodes_by_type(root, ["element"]):
            ...
            if "-" in tag_name and tag_name not in ["input", "meta", "link"]:
                entity = Entity(
                    ...
                    file_path=file_path,
                    line_number=element.start_point[0] + 1,
                    metadata={"type": "html_component", "tag": tag_name},
                )
                entities.append(entity)

            # Check for elements with data-component attribute
            component_attr = self._get_attribute_value(
                element, "data-component", content
            )
            if component_attr:
                entity = Entity(
    ...
    def _extract_links(
            ...
            if tag_name == "a":
                ...
                if href and not href.startswith(("#", "javascript:", "mailto:")):
                    relation = RelationFactory.create_imports_relation(
                        importer=str(file_path), imported=href, import_type="html_link"
                    )
                    relations.append(relation)
        ...
        for script_element in self._find_nodes_by_type(root, ["script_element"]):
            ...
            if src and not src.startswith(("http:", "https:", "//")):
                relation = RelationFactory.create_imports_relation(
                    importer=str(file_path), imported=src, import_type="html_resource"
                )
                relations.append(relation)
        return relations
                ...
                for class_name in classes:
                    if class_name.strip():
                        relation = Relation(
                            ...
                            context="HTML element uses CSS class",
    ...
    def _get_attribute_value(
        self, element: Node, attr_name: str, content: str
    ) -> str | None:
    ...
    def _extract_inline_css_entities(
        self, root: Node, content: str, file_path: Path
    ) -> list[Entity]:
        ...
        for style_element in self._find_nodes_by_type(root, ["style_element"]):
            ...
            if style_content and style_content.strip():
                ...
                try:
                    ...
                    css_entities = []
                    css_entities.extend(
                        self._extract_css_class_definitions(
                            css_tree.root_node,
                            style_content,
                            file_path,
                            style_element.start_point[0] + 1,
                        )
                    )
                    css_entities.extend(
                        self._extract_css_id_definitions(
                            css_tree.root_node,
                            style_content,
                            file_path,
                            style_element.start_point[0] + 1,
                    ...
                    css_entities.extend(
                        self._extract_css_variable_definitions(
                            css_tree.root_node,
                            style_content,
                            file_path,
                            style_element.start_point[0] + 1,
                    ...
                    basic_entities = self._extract_basic_css_patterns(
                        style_content, file_path, style_element.start_point[0] + 1
                    )
    ...
    def _extract_css_class_definitions(
        self, root: Node, css_content: str, file_path: Path, base_line: int
    ) -> list[Entity]:
    ...
    def _extract_css_id_definitions(
        self, root: Node, css_content: str, file_path: Path, base_line: int
    ) -> list[Entity]:
    ...
    def _extract_css_variable_definitions(
        self, root: Node, css_content: str, file_path: Path, base_line: int
    ) -> list[Entity]:
    ...
    def _extract_css_selectors_from_rule(
        self, rule: Node, css_content: str
    ) -> list[str]:
    ...
    def _extract_basic_css_patterns(
        self, css_content: str, file_path: Path, base_line: int
    ) -> list[Entity]:
