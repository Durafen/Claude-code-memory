...
class YAMLParser(TreeSitterParser):
    ...
    def parse(self, file_path: Path) -> ParserResult:
        ...
        try:
            ...
            if self._has_syntax_errors(tree):
                result.errors.append(f"YAML syntax errors in {file_path.name}")  # type: ignore[union-attr]
            ...
            if file_type == "github_workflow":
                special_entities, special_relations = self._handle_github_workflow(
                ...
                special_entities, special_relations = self._handle_docker_compose(
                    tree.root_node, content, file_path
                )
                entities.extend(special_entities)
                relations.extend(special_relations)
            elif file_type == "kubernetes":
                special_entities, special_relations = self._handle_kubernetes(
                    tree.root_node, content, file_path
                )
                entities.extend(special_entities)
                relations.extend(special_relations)
                ...
                generic_entities = self._extract_generic_structure(
                    tree.root_node, content, file_path
                )
            ...
            result.errors.append(f"YAML parsing failed: {e}")  # type: ignore[union-attr]
    ...
    def _handle_github_workflow(
        ...
        workflow_name = (
            self._extract_yaml_value(root, "name", content) or file_path.stem
        )
        ...
        for job_name in jobs:
            ...
            relation = RelationFactory.create_contains_relation(
                f"Workflow: {workflow_name}", f"Job: {job_name}"
            )
            relations.append(relation)

        return entities, relations

    def _handle_docker_compose(
        self, root: Node, content: str, file_path: Path
    ) -> tuple[list[Entity], list[Relation]]:
    ...
    def _handle_kubernetes(
        ...
        if kind and name:
            entity = Entity(
                ...
                observations=[f"Kubernetes {kind}: {name}", f"File: {file_path.name}"],
                file_path=file_path,
                line_number=1,
